...
const RecordList = () => {
  ...
  const [pageSize, setPageSize] = useState(
    myRecordList?.pagination?.pageSize ?? 25
  );
  ...
  const getRecords = async (meta) => {
    ...
    const existSearchParams = { ...searchFormValues };

    Object.keys(existSearchParams).forEach((key) => {
      if (existSearchParams[key] === null) delete existSearchParams[key];
    });
  ...
  }, [sortItems, searchFormValues, pageSize]);
  ...
  const deleteRecord = async (recordId) => {
    if (window.confirm("記録を削除しますか？")) {
      ...
      if (res.ok) {
        ...
        CopiedRecordList.results.splice(index, 1);
  ...
  const pageChange = (pageData) => {
    ...
    const meta = {
      page: pageNumber,
      pageSize: myRecordList.pagination.pageSize,
    };
    getRecords(meta);
  };
  ...
  return (
    <div className="relative z-0">
      ...
      {myRecordList?.results?.length > 0 && (
        <>
          <div className="flex justify-center">
            <div className="flex justify-center w-4/5 relative mb-2">
              <ReactPaginate
                pageCount={myRecordList.pagination?.pageCount}
              ...
              <div className="absolute -top-7 right-0">
                <SelectBox
                  ...
                  handleSelect={(e) => setPageSize(Number(e))}
          ...
          <div className="flexCol items-center">
            <div className="w-4/5 border-2 border-gray-700 rounded-xl relative">
              <div>
                ...
                <div>
                  {myRecordList?.results.length > 0 &&
                    myRecordList.results.map((record, index) => (
                      <div
                        key={record.id}
                        onClick={() => {
                        ...
                        className={
                          editRecord?.id === record.id
                            ...
                            : index + 1 === myRecordList.results.length
                            ? tableRowStyle
